...
DefaultLookAndFeel::DefaultLookAndFeel() {
  ...
  setColour(juce::PopupMenu::headerTextColourId, juce::Colour(0xff333333));
  setColour(juce::PopupMenu::highlightedBackgroundColourId, juce::Colour(0xff111111));
  setColour(juce::PopupMenu::highlightedTextColourId, juce::Colour(0xffcccccc));
...
void DefaultLookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height,
                                          float slider_pos, float min, float max,
                                          const juce::Slider::SliderStyle style, juce::Slider& slider) {
  static const juce::DropShadow thumb_shadow(juce::Colour(0x88000000), 3, juce::Point<int>(-1, 0));
  ...
  juce::Colour slider_color(0xff888888);
  juce::Colour lighten_color(0x55ffffff);
  juce::Colour thumb_color(0xffffffff);

  if (!active) {
    slider_color = juce::Colour(0xff424242);
    thumb_color = juce::Colour(0xff888888);
    lighten_color = juce::Colour(0x22ffffff);
  }
  ...
  if (style == juce::Slider::SliderStyle::LinearBar) {
    ...
    fillSplitHorizontalRect(g, 0.0f, w, h, juce::Colours::transparentBlack);
    ...
    thumb_shadow.drawForRectangle(g, juce::Rectangle<int>(pos + 0.5f, 0, 2, h));
    ...
    g.setColour(juce::Colour(0xff2a2a2a));
    fillSplitVerticalRect(g, 0.0f, h, w, juce::Colours::transparentBlack);
    ...
    thumb_shadow.drawForRectangle(g, juce::Rectangle<int>(0, pos + 0.5f, w, 2));
    g.setColour(thumb_color);
    g.fillRect(0.0f, pos, w, 2.0f);
  }
}

void DefaultLookAndFeel::drawLinearSliderThumb(juce::Graphics& g, int x, int y, int width, int height,
                                               float slider_pos, float min, float max,
                                               const juce::Slider::SliderStyle style, juce::Slider& slider) {
...
void DefaultLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height,
  ...
  if (slider.getInterval() == 1) {
    ...
    juce::Rectangle<float> text_bounds(1.0f + width * (1.0f - TEXT_W_PERCENT) / 2.0f,
    ...
    g.drawFittedText(juce::String(slider.getValue()), text_bounds.getSmallestIntegerContainer(),
                     juce::Justification::horizontallyCentred | juce::Justification::bottom, 1);
  }
...
void DefaultLookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& button,
                                          bool hover, bool is_down) {
  static const juce::DropShadow shadow(juce::Colour(0x88000000), 1.0f, juce::Point<int>(0, 0));
  ...
  float stroke_width = 2.0f * full_radius * stroke_percent;
  juce::PathStrokeType stroke_type(stroke_width, juce::PathStrokeType::beveled, juce::PathStrokeType::rounded);
  ...
  juce::Rectangle<int> bar_shadow_rect(full_radius - 1.0f, padding, 2.0f, full_radius - padding);
...
void DefaultLookAndFeel::drawButtonBackground(juce::Graphics& g, juce::Button& button,
  ...
  if (button.isEnabled())
    ...
    g.fillAll(juce::Colour(0xff484848));
...
void DefaultLookAndFeel::drawButtonText(juce::Graphics& g, juce::TextButton& button,
                                        bool hover, bool is_down) {
...
void DefaultLookAndFeel::fillHorizontalRect(juce::Graphics& g, float x1, float x2, float height) {
  float x = std::min(x1, x2);
  float width = fabsf(x1 - x2);
  g.fillRect(x, 0.0f, width, height);
}

void DefaultLookAndFeel::fillVerticalRect(juce::Graphics& g, float y1, float y2, float width) {
  float y = std::min(y1, y2);
  float height = fabsf(y1 - y2);
  g.fillRect(0.0f, y, width, height);
}

void DefaultLookAndFeel::fillSplitHorizontalRect(juce::Graphics& g, float x1, float x2, float height,
                                                 juce::Colour fill_color) {
...
void DefaultLookAndFeel::fillSplitVerticalRect(juce::Graphics& g, float y1, float y2, float width,
                                               juce::Colour fill_color) {
